    ...
    */
    ...
    void process_application_copy(){
        ...
        for (auto index = 0; index < receive_available_map.size(); index++){
            ...
            if (pkt_difference == receive_connection_difference && (pkt_offset == 0 || pkt_offset == 48) && pkt_len < 1440){
                recvCQ.copy(pkt_difference, pkt_offset, receive_message[index].iov[1].iov_base, pkt_len);
                std::cout<<"copycount:"<<pkt_len;
                return;
            }
        }
        /*
