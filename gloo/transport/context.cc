...
std::shared_ptr<transport::Pair>& Context::getPair(int rank) {
...
Context::Tally& Context::LazyTally::get() {
  // std::cout<<"1 Context::LazyTally::get():"<<vec_.size()<<std::endl;
  initialize_iterator();
  // std::cout<<"2 Context::LazyTally::get():"<<vec_.size()<<std::endl;
  if (it_ == vec_.end()) {
